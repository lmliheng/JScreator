...
async function submitEmail() {
  ...
  try {
    const { token, user } = await emailLogin(email.value.trim(), code.value.trim())
...
// GitHub 授权回调后，后端会重定向到 /login?token=...&username=...
onMounted(() => {
...
<template>
  <div class="mx-auto max-w-md px-4 py-12">
    <div class="card">
      ...
      <p class="mt-2 text-sm text-muted">使用用户名、邮箱或验证码登录。</p>
      ...
      <button type="button" class="btn-accent w-full" @click="githubLogin">
